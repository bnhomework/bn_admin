  ...
  <svg style="width: 100%;height: 100%;min-height: 800px;min-width: 800px">
    <template v-for="r in relationships">
      ...
      <g fill="none" class="bn-line-action">
        ...
        <text :x="getActionPoint(r).x" :y="getActionPoint(r).y" class="bn-icon bn-line-action-item" font-size="35" :fill="lineColor" transform="translate(15,20)"  @click="removeRelationship(r)">
          &#xe60c;
        </text>
      </g>
    </template>
...
export default {
  ...
  methods: {
    ...
    getActionPoint(r) {
      return this.calcLine(r).m2;
    },
    getActionPath(r) {
      var m2 = this.calcLine(r).m2
      return `M ${m2.x} ${m2.y} A 3 3 0 1 0 ${m2.x} ${m2.y-1}`
    },
    ...
    drawRightToLeft(r) {
      ...
      return {
        line: `M ${from.x-5} ${from.y} L ${m1.x} ${m1.y} L ${m2.x} ${m2.y} L ${to.x} ${to.y}`,
        arrow: arrow,
        m2: m2
      };
    },
